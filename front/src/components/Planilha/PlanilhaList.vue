...
const SCollapsibleRecursivo = {
    ...
    render(h) {
        ...
        if (this.isObject(self.planilha) && typeof self.planilha.itens === 'undefined') {
            return h('VList',
                { props: { expand: true, value: self.toggleExpand(this.planilha, self.contador) }, attrs: { expands: 'expands' } },
                Object.keys(this.planilha).map((key) => {
                    if (self.isObject(self.planilha[key])) {
                        ...
                        return h('VListGroup', { props: { value: true } },
                            [
                                h('template', { slot: 'activator' },
                                    [
                                        h('VListTile',
                                            [
                                                h('VListTileTitle',
                                                    ...
                                                    [
                                                        h('i',
                                                            { class: `material-icons smt-2 spl-${self.contador * 1}` },
                                                            [self.getHeader(self.contador).icon]),
                                                        h('span', { class: 'ml-2 smt-2' }, key),
                                                        h('VSpacer'),
                                                        badgeHeader,
                                                    ],
                                                ),
